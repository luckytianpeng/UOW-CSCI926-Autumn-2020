...
% Configuration:
THRESH = 0.4;       % probability, for YOLO
...
SHOW_IMAGE = true;
SAVE_IMAGE = true;
IMAGE_DIR = 'output_new_signs';

CSV_FILE = 'output_new_signs.csv';
...
f = fopen(CSV_FILE,'wt');
fprintf(f, 'video,frame,oldclass,signname,newclass\n');
fclose(f);
f = fopen(CSV_FILE,'a+');
...
for s_i = 1:length(sign_files)
    ...
    [filepath,signname,ext] = fileparts(s_full_path);
    ...
    for v_i = 1:length(video_files)
        ...
        while hasFrame(v)
            ...
            % [480, 704] is the size of input of the pretrained YOLO
            outputImage = imresize(img, [YOLO_HEIGHT,YOLO_WIDTH]);

            [boundingBoxes,classIndices] = tsdr_predict_thresh_mex(outputImage, THRESH);
            
            % proc every bounding box:
            for i = 1:size(boundingBoxes,1)
                oldClassRec = CLASS_NAMES{classIndices(i)};
                
                outputImage = rst(outputImage, boundingBoxes, sign, alpha);

                [newBoundingBoxes,newClassIndices] = tsdr_predict_thresh_mex(outputImage, THRESH);

                for ii = 1:size(newBoundingBoxes,1)
                    if DRAW_CLASS_TEXT
                        newClassRec = CLASS_NAMES{newClassIndices(ii)};
                        ymin = newBoundingBoxes(ii,2);xmin=newBoundingBoxes(ii,1);xmax=xmin+newBoundingBoxes(ii,3);
                        outputImage = insertText(outputImage,[xmax ymin-20],newClassRec,'FontSize',20,'TextColor','red');
                    end

                    % draw bounding box:
                    if DRAW_BOUNDING_BOX
                        outputImage = insertShape(outputImage,'Rectangle', newBoundingBoxes, 'LineWidth', 3);
                    end

                    fprintf(f, '%s,%d,%s,%s,%s\n', video_files(v_i), count, oldClassRec, signname, newClassRec);
                end
            end
            ...
            if SAVE_IMAGE
                si(outputImage, 'png', strcat(IMAGE_DIR, '\', signname, '_', video_files(v_i), '_', num2str(count)));
            end
        end % frame
    end % video
end % sign
...
function [outputIamge] = rst(image, boundingBox, sign, alpha)
    ...
    if YOLO_HEIGHT >= a_y2 && YOLO_WIDTH >= a_x2
